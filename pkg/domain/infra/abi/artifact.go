...
func (ir *ImageEngine) ArtifactInspect(ctx context.Context, name string, opts entities.ArtifactInspectOptions) (*entities.ArtifactInspectReport, error) {
	artStore, err := store.NewArtifactStore(ir.Libpod.StorageConfig().GraphRoot, ir.Libpod.SystemContext())
	if err != nil {
		return nil, err
	}
...
func (ir *ImageEngine) ArtifactAdd(ctx context.Context, path, name string, opts entities.ArtifactAddoptions) (*entities.ArtifactAddReport, error) {
	...
	newBlobDigest, err := artStore.Add(ctx, name, path, opts.ArtifactType)
	if err != nil {
		return nil, err
	}
